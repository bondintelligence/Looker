view: risk_predicted_muni {
  derived_table: {
    sql: SELECT CENTROID_ID as nearestCentroidID,
      NEAREST_CENTROIDS_DISTANCE[ORDINAL(CENTROID_ID)].DISTANCE AS nearestCentroidDistance,
      _20th_Percentile_Income,
      Days_between_maturity_date_and_trade_date,
      Median_Gross_Rent_dollars_MH,__Non_Hispanic_White,
      Percent_bachelor_s_degree_or_higher_dem, Poverty_Rate_eco, Ratings1, Ratings2, Ratings3,
      Trade_Date,
      Unemployment_Rate_eco, Yield_at_Issue,
      ...
      (
      SELECT
      ((CASE WHEN {% parameter _20th_Percentile_Income %} IS NULL THEN
      ((_20th_Percentile_Income * POWER(4.08144607e+07, 0.5)) + 2.40765872e+04)
      ELSE {% parameter _20th_Percentile_Income%}
      END) - 2.40765872e+04)/POWER(4.08144607e+07, 0.5) AS _20th_Percentile_Income,
      ((CASE WHEN {% parameter Days_between_maturity_date_and_trade_date %} IS NULL THEN
      ((Days_between_maturity_date_and_trade_date * POWER(8.81677773e+06, 0.5)) + 4.29082934e+03)
      ELSE {% parameter Days_between_maturity_date_and_trade_date %}
      END) - 4.29082934e+03)/POWER(8.81677773e+06, 0.5) AS
      Days_between_maturity_date_and_trade_date,
      ((CASE WHEN {% parameter Median_Gross_Rent_dollars_MH%} IS NULL THEN
      ((Median_Gross_Rent_dollars_MH * POWER(7.37373826e+04 , 0.5)) + 9.93629670e+02)
      ...
      POWER(2.05444267e+01 , 0.5)) + 1.30039056e+01)
      ...
      ((CASE WHEN {% parameter Unemployment_Rate_eco %} IS NULL THEN
      ((Unemployment_Rate_eco * POWER(3.04280246e+00 , 0.5)) + 5.37110987e+00)
      ...
      ((CASE WHEN {% parameter
      _10_Year_Treasury_Constant_Maturity_Rate_Percent_Daily_Not_Seasonally_Adjusted %} IS NULL THEN
      ((_10_Year_Treasury_Constant_Maturity_Rate_Percent_Daily_Not_Seasonally_Adjusted *
      POWER(2.27818760e-01 , 0.5)) + 2.39064800e+00)
      ELSE {% parameter
      _10_Year_Treasury_Constant_Maturity_Rate_Percent_Daily_Not_Seasonally_Adjusted%}
      END) - 2.39064800e+00)/POWER(2.27818760e-01, 0.5) AS
      _10_Year_Treasury_Constant_Maturity_Rate_Percent_Daily_Not_Seasonally_Adjusted,
...
#Dimensions
  ...
  dimension: _20th_percentile_income_ {
    type: number
    sql: ROUND(((${TABLE}._20th_Percentile_Income * POWER(4.08144607e+07, 0.5)) +
      2.40765872e+04),2) ;;
    hidden: yes
  }
  ...
  dimension: days_between_maturity_date_and_trade_date_ {
    type: number
    sql: ROUND(((${TABLE}.Days_between_maturity_date_and_trade_date * POWER(8.81677773e+06,
      0.5)) + 4.29082934e+03),2) ;;
    hidden: yes
  }

  dimension: median_gross_rent_dollars_mh_ {
    type: number
    sql: ROUND(((${TABLE}.Median_Gross_Rent_dollars_MH * POWER(7.37373826e+04 , 0.5)) +
      9.93629670e+02),2) ;;
    hidden: yes
  }
  ...
  dimension: __non_hispanic_white_ {
    type: number
    sql: ROUND(((${TABLE}.__Non_Hispanic_White * POWER(2.92049843e+02 , 0.5)) +
      6.13443237e+01),2);;
    hidden: yes
  }
  ...
  dimension: percent_bachelor_s_degree_or_higher_dem_ {
    type: number
    sql: ROUND(((${TABLE}.Percent_bachelor_s_degree_or_higher_dem * POWER(6.93219238e+01 ,
      0.5)) + 3.27578339e+01),2) ;;
    hidden: yes
  }
  ...
  dimension: unemployment_rate_eco_ {
    type: number
    sql: ROUND(((${TABLE}.Unemployment_Rate_eco * POWER(3.04280246e+00 , 0.5)) +
      5.37110987e+00),2) ;;
    hidden: yes
  }
  ...
  dimension: _10_year_treasury_constant_maturity_rate_percent_daily_not_seasonally_adjusted_ {
    type: number
    sql:
    ROUND(((${TABLE}._10_Year_Treasury_Constant_Maturity_Rate_Percent_Daily_Not_Seasonally_Adjusted
    * POWER(2.27818760e-01 , 0.5)) + 2.39064800e+00),2) ;;
    hidden: yes
  }
...
#Measures
  ...
  measure: evaluated_risk {
    ...
    WHEN ${nearest_centroid_id_} = 2 OR ${nearest_centroid_id_} = 4 OR ${nearest_centroid_id_} = 5
    THEN "Medium"
