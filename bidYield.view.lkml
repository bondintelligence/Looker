view: bidYield {
  derived_table: {
    sql: WITH original AS
      ...
      SELECT CUSIP, CAST(Date AS TIMESTAMP) as `Date`, Yield_Data_.Category as
      `Category`, Yield_Data_.Yield as `Yield` FROM original
      CROSS JOIN UNNEST (original.Yield_Data) AS Yield_Data_ WHERE Category = 'BidYTM'
      ;;
  }
  measure: count {
    type: count
  ...
  set: detail {
    fields: [cusip, date_time, category, bid_yield]
  }
}
